...
export function HeroSlidesEditor({ formData, setFormData, langTab }: HeroSlidesEditorProps) {
  ...
  const [libraryPickerType, setLibraryPickerType] = useState<'image' | 'video' | null>(null);
  ...
  const addFromLibrary = (asset: MediaLibraryAsset) => {
    if (asset.assetType === 'video') {
      setSlides([
        ...slides,
        makeSlide('video', asset.url, {
      ...
      setSlides([
        ...slides,
        makeSlide('image', asset.url, { cloudinaryPublicId: asset.publicId }),
      ]);
    }
    setLibraryPickerType(null);
    setError('');
  };
    ...
    <>
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-xs leading-relaxed text-slate-500">
              Добавляйте фото и фоновые MP4/WebM-видео. Пустые поля текста слайда используют общий H1, подзаголовок и CTA ниже. Фото оптимизируются в WebP, медиа хранится в Cloudinary и регистрируется в медиатеке.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() => setLibraryPickerType('image')}
              disabled={uploading !== null}
              className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-white px-4 py-2.5 text-xs font-bold text-indigo-700 hover:bg-indigo-50 disabled:opacity-60"
            >
              <Library className="h-4 w-4" /> Фото из медиатеки
            </button>
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={() => setLibraryPickerType('video')}
              disabled={uploading !== null}
              className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-60"
            >
              <Library className="h-4 w-4" /> Видео из медиатеки
            </button>
          </div>
        </div>
      ...
      {libraryPickerType && (
        <MediaLibraryPicker
          ...
          title={libraryPickerType === 'image' ? 'Добавить фото в Hero' : 'Добавить видео в Hero'}
          onClose={() => setLibraryPickerType(null)}
