          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-6 space-y-6">
              <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-semibold uppercase tracking-wider">
                ...
                <span>{l("Швидкий старт проєкту", "Быстрый старт проекта")}</span>
              </div>

              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-white leading-tight">
                {l("Отримайте розрахунок кошторису та концепцію за 15 хвилин", "Получите расчет сметы и концепцию за 15 минут")}
              </h2>
              ...
              <div className="space-y-4 pt-4 text-xs sm:text-sm text-slate-400">
                <div className="flex items-center space-x-3">
                  ...
                  <span>{l("Відповідь протягом 15 хвилин у робочий час", "Ответ в течение 15 минут в рабочее время")}</span>
                </div>
                <div className="flex items-center space-x-3">
                  <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0" />
                  <span>{l("Фіксований кошторис та конфіденційність за NDA", "Фиксированная смета и конфиденциальность по NDA")}</span>
                </div>
                <div className="flex items-center space-x-3">
                  <Phone className="w-4 h-4 text-indigo-400 shrink-0" />
                  <span>
                    {l("Прямий телефонний зв'язок: ", "Прямая телефонная связь: ")}
              ...
              {isSuccess ? (
                <div className="text-center py-10 space-y-4">
                  ...
                  <p className="text-xs sm:text-sm text-slate-300 max-w-sm mx-auto leading-relaxed">
                    {l("Ми вже аналізуємо параметри вашого проєкту і зателефонуємо вам протягом 15 хвилин.", "Мы уже анализируем параметры вашего проекта и перезвоним вам в течение 15 минут.")}
                  </p>
                  <button
                  ...
                  >
                    {l("Надіслати ще один запит", "Отправить еще один запрос")}
                  </button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                      {l("Ваше ім'я або компанія *", "Ваше имя или компания *")}
                    </label>
                    <input
                      required
                      type="text"
                      placeholder={l("Наприклад: Артем / Event Agency", "Например: Артем / Event Agency")}
                      value={name}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                      {l("Напрямок, що цікавить", "Интересующее направление")}
                    </label>
                    <div className="grid grid-cols-3 gap-2">
                      {[
                        { id: 'LIVE', label: l("LIVE Стрім", "LIVE Стрим") },
                        { id: 'VIDEO', label: l("Відео", "Видео") },
                        { id: 'CONSTRUCTION', label: l("Будівництво / 3D", "Стройка / 3D") }
                      ].map((item) => (
                        <button
                          key={item.id}
                          type="button"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                      {l("Короткий опис завдання або дата (за бажанням)", "Краткое описание задачи или дата (по желанию)")}
                    </label>
                    <textarea
                      rows={3}
                      placeholder={l("Наприклад: Турнір з дзюдо 14 жовтня, потрібно 4 камери та повтори...", "Например: Турнир по дзюдо 14 октября, нужно 4 камеры и повторы...")}
                      value={comment}
                  ...
                  >
                    <span>
                      {isSubmitting 
                        ? (l("Відправка...", "Отправка...")) 
                        : (l("Отримати розрахунок та консультацію", "Получить расчет и консультацию"))}
                    </span>
                    <Send className="w-4 h-4" />
                  </button>

                  <p className="text-[11px] text-slate-500 text-center">
                    {l("Натискаючи кнопку, ви погоджуєтеся на обробку контактних даних для зв'язку.", "Нажимая кнопку, вы соглашаетесь на обработку контактных данных для связи.")}
                  </p>
                </form>
              )}
            </div>
