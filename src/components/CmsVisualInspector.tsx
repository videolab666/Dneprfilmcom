...
function orderedStructureItems(group: RuntimeGroup, config: ReturnType<typeof normalizeFullPageCms>) {
...
function buildCandidates(
  ...
  const structureCopyIds = new Set(groups.flatMap(group => group.items.flatMap(item => item.fields.map(field => field.copyId).filter(Boolean) as string[])));
    ...
    for (const { catalog, patch } of orderedStructureItems(group, config)) {
      ...
      for (const field of catalog.fields) {
        ...
        output.push({
          ...
          detail: `${group.source}:${group.line}`,
          kind: valueKind(field.key, value),
          value,
          priority: 0,
          groupId: group.id,
          itemId: patch.id,
          fieldKey: field.key,
          field,
          defaultValue: (field.scope || 'locale') === 'common' ? field.common : field[locale],
        });
      }
    }
  }
  ...
  for (const entry of EDITABLE_COPY_CATALOG) {
    ...
    output.push({
      ...
      sourceType: 'copy',
      label: entry.label,
      detail: `${entry.source}:${entry.line}`,
      kind: valueKind(entry.label, value),
      value,
      priority: 10,
      copyEntry: entry,
      defaultValue: entry[locale],
    });
  }
  ...
  for (const base of LOCALIZED_SETTING_BASES) {
    ...
    output.push({
      id: `setting:${key}`,
      sourceType: 'setting',
      label: `Site Settings → ${base}`,
      detail: `site_settings/global · ${key}`,
      kind: valueKind(base, display),
      value: display,
      priority: 30,
      settingKey: key,
      defaultValue: (rawSettings as unknown as Record<string, StructureScalar>)[key],
    });
  }

  for (const [key, value] of Object.entries(rawSettings as unknown as Record<string, unknown>)) {
    if (LOCALIZED_SETTING_BASES.has(key) || /_(uk|en)$/.test(key) || key === 'id' || key === 'fullPageCms' || key === 'heroSlides') continue;
    if (typeof value !== 'string' || !value.trim()) continue;
    output.push({
      id: `setting:${key}`,
      sourceType: 'setting',
      label: `Site Settings → ${key}`,
      detail: 'site_settings/global',
  ...
  const textFields = ['badge', 'title', 'subtitle', 'ctaPrimaryText', 'ctaSecondaryText'] as const;
  const commonHeroFields = ['url', 'mobileUrl', 'posterUrl', 'ctaPrimaryLink', 'ctaSecondaryLink', 'objectPosition'] as const;
  for (const slide of rawSettings.heroSlides || []) {
    for (const field of textFields) {
      const storageField = locale === 'ru' ? field : `${field}_${locale}` as keyof HeroSlide;
      const value = slide[storageField];
      if (typeof value !== 'string' || !value.trim()) continue;
      output.push({
        id: `hero:${slide.id}:${String(storageField)}`,
        sourceType: 'hero',
        label: `Hero → ${slide.id} → ${field}`,
        detail: `heroSlides.${slide.id}.${String(storageField)}`,
        kind: 'text',
        value,
        priority: 20,
        heroId: slide.id,
        heroField: String(storageField),
        defaultValue: value,
      });
    }
    for (const field of commonHeroFields) {
      const value = slide[field];
      if (typeof value !== 'string' || !value.trim()) continue;
      output.push({
        id: `hero:${slide.id}:${field}`,
        sourceType: 'hero',
        label: `Hero → ${slide.id} → ${field}`,
        detail: `heroSlides.${slide.id}.${field}`,
  ...
  for (const block of blocks) {
    ...
    if (block.title) {
      output.push({
        ...
        kind: 'text', value: block.title, priority: 25, blockId: block.id, blockPath: localizedBlockPath('title', locale, false), defaultValue: block.title,
      });
    }
    for (const [key, value] of Object.entries(block.config || {})) {
      ...
      output.push({
        id: `block:${block.id}:config.${key}`,
        sourceType: 'block',
        label: `Page Builder → ${block.id} → ${key}`,
        detail: block.type,
        kind: valueKind(key, value),
        value,
        priority: 25,
        blockId: block.id,
        blockPath: localizedBlockPath(`config.${key}`, locale, common),
        defaultValue: value,
      });
    }
  }
...
function ensurePatch(draft: FullPageCmsContent, group: RuntimeGroup, itemId: string): StructureItemOverride {
  ...
  if (!patch) {
    ...
    patch = { id: itemId, enabled: true, order: Math.max(0, index) * 10 };
    structure.items.push(patch);
  }
  return patch;
}

function inputValue(value: StructureScalar): string {
...
export function CmsVisualInspector() {
  ...
  const bind = (element: HTMLElement, candidate: Candidate) => {
    const current = bindingsRef.current.get(element) || [];
    if (!current.some(item => item.id === candidate.id)) bindingsRef.current.set(element, [...current, candidate].sort((a, b) => a.priority - b.priority));
  };
  ...
  useEffect(() => {
    ...
    const rebuild = () => {
      ...
      const structureItems = new Map(candidates.filter(item => item.sourceType === 'structure-item').map(item => [`${item.groupId}:${item.itemId}`, item]));

      for (const candidate of candidates) {
        if (candidate.sourceType === 'structure-item') continue;
        ...
        if ((candidate.kind === 'url' || candidate.kind === 'media') && typeof candidate.value === 'string') {
          byAttribute.set(candidate.value, [...(byAttribute.get(candidate.value) || []), candidate]);
        }
      }
      ...
      document.querySelectorAll<HTMLElement>('h1,h2,h3,h4,h5,h6,p,span,a,button,li,dt,dd,strong,em,small,label').forEach(element => {
        ...
        if (matches) matches.forEach(candidate => bind(element, candidate));
      });

      document.querySelectorAll<HTMLElement>('a[href],img[src],video[src],source[src],source[srcset]').forEach(element => {
        if (element.closest('[data-cms-inspector-ui]')) return;
        const values = ['href', 'src', 'srcset'].map(name => element.getAttribute(name)).filter(Boolean) as string[];
        values.forEach(attribute => (byAttribute.get(attribute) || []).forEach(candidate => bind(element, candidate)));
      });

      document.querySelectorAll<HTMLElement>('[data-cms-structure-item][data-cms-structure-group]').forEach(element => {
        const key = `${element.dataset.cmsStructureGroup}:${element.dataset.cmsStructureItem}`;
        const candidate = structureItems.get(key);
    ...
    try {
      ...
      if (selected.sourceType === 'copy' && selected.copyEntry) {
        await mutateDraft(draft => {
          ...
          if (nextValue === entry[locale]) delete current[locale];
          else current[locale] = String(nextValue ?? '');
        ...
        await mutateDraft(draft => {
          ...
          if ((field.scope || 'locale') === 'common') {
            ...
            if (nextValue === field.common) delete patch.common[field.key];
            else patch.common[field.key] = nextValue;
            ...
            if (String(nextValue ?? '') === (field[locale] || '')) delete current[locale];
            else current[locale] = String(nextValue ?? '');
  ...
  const mutateStructure = async (operation: 'toggle' | 'up' | 'down' | 'duplicate' | 'delete' | 'layout', layout?: string) => {
    ...
    await mutateDraft((draft, canonical) => {
      const structure = ensureStructure(draft, group);
      const currentConfig = canonical.draft;
      const order = orderedStructureItems(group, currentConfig).map(entry => entry.patch.id);
      let patch = structure.items.find(item => item.id === itemId);
      if (!patch) patch = ensurePatch(draft, group, itemId);
      if (operation === 'toggle') patch.enabled = patch.enabled === false;
      if (operation === 'layout') structure.layout = (layout || 'auto') as never;
      ...
      if (operation === 'up' || operation === 'down') {
        ...
        if (index >= 0 && target >= 0 && target < order.length) {
          ...
          order.forEach((id, idx) => {
            const item = structure.items.find(entry => entry.id === id) || ensurePatch(draft, group, id);
            item.order = idx * 10;
          });
        }
      }
        ...
        for (const field of current.catalog.fields) {
          if ((field.scope || 'locale') === 'common') {
            ...
            for (const lang of LANGS) {
              ...
              const currentValue = fieldValue(field, lang, currentConfig, current.patch);
              locales[lang]![field.key] = String(currentValue ?? '');
            }
          }
        }
  ...
  const structurePatch = structureContext?.group ? config.structures[structureContext.group.id]?.items.find(item => item.id === structureContext.itemId) : undefined;
  const structureLayout = structureContext?.group ? config.structures[structureContext.group.id]?.layout || 'auto' : 'auto';
  ...
  return (
    <>
      ...
      <div data-cms-inspector-ui className="fixed bottom-4 left-4 z-[9998] flex items-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 text-xs font-black text-white shadow-2xl">
        <MousePointer2 className="h-4 w-4 text-fuchsia-400" />Visual CMS Inspector 2.0 · {page} · {locale.toUpperCase()}
      </div>
      {selected && <aside data-cms-inspector-ui className="fixed right-4 top-20 z-[9999] max-h-[calc(100vh-6rem)] w-[min(460px,calc(100vw-2rem))] overflow-y-auto rounded-3xl border border-slate-700 bg-slate-950 p-5 text-white shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div><div className="text-[10px] font-black uppercase tracking-wider text-fuchsia-400">{selected.sourceType} · {selected.kind}</div><div className="mt-1 text-sm font-black">{selected.label}</div><div className="mt-1 font-mono text-[9px] text-slate-500">{selected.detail}</div></div>
          <button onClick={() => setSelectedCandidates([])} className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-white"><X className="h-4 w-4" /></button>
        </div>
        ...
        {user && structureContext?.group && <div className="mt-5 border-t border-slate-800 pt-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button onClick={() => void mutateStructure('up')} className="rounded-xl border border-slate-700 px-3 py-2 text-xs font-bold hover:bg-slate-800">↑ Выше</button>
            <button onClick={() => void mutateStructure('down')} className="rounded-xl border border-slate-700 px-3 py-2 text-xs font-bold hover:bg-slate-800">↓ Ниже</button>
            <button onClick={() => void mutateStructure('toggle')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-700 px-3 py-2 text-xs font-bold hover:bg-slate-800">{structurePatch?.enabled === false ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}{structurePatch?.enabled === false ? 'Показать' : 'Скрыть'}</button>
            <button onClick={() => void mutateStructure('duplicate')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-700 px-3 py-2 text-xs font-bold hover:bg-slate-800"><Copy className="h-3.5 w-3.5" />Дублировать</button>
          </div>
