...
function SectionShell({ block, children }: { block: BuilderSiteBlock; children: React.ReactNode }) {
  return (
...
function TimelineSection({ block }: { block: BuilderSiteBlock }) {
  ...
  const horizontal = block.config.timelineOrientation === 'horizontal';
  if (horizontal) {
    return (
      <SectionShell block={block}>
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {items.map((item, index) => (
            <article key={item.id || index} className="relative rounded-3xl border border-slate-200 bg-white p-6 text-slate-900 shadow-sm">
...
function DownloadsSection({ block }: { block: BuilderSiteBlock }) {
  const items = block.config.downloadItems || [];
    ...
    <SectionShell block={block}>
      <div className={cards ? 'grid gap-4 md:grid-cols-2 lg:grid-cols-3' : 'space-y-3'}>
        {items.map((item, index) => {
          const content = (
            <>
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600"><FileText className="h-6 w-6" /></div>
              <div className="min-w-0 flex-1">
                <h3 className="font-black text-slate-900">{item.title || item.fileName || `Файл ${index + 1}`}</h3>
                {item.description && <p className="mt-1 text-sm leading-6 text-slate-500">{item.description}</p>}
                <div className="mt-2 flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-wider text-slate-400">{item.fileType && <span>{item.fileType}</span>}{item.fileSize && <span>· {item.fileSize}</span>}</div>
              </div>
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-950 text-white transition group-hover:bg-indigo-600"><Download className="h-4 w-4" /></span>
            </>
          );
    ...
    <SectionShell block={block}>
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white text-slate-900 shadow-sm">
        <div className="flex gap-1 overflow-x-auto border-b border-slate-200 bg-slate-50 p-2">
          {tabs.map((item, index) => <button key={item.id || index} type="button" onClick={() => setActiveId(item.id)} className={`whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-black transition ${active?.id === item.id ? 'bg-slate-950 text-white shadow-sm' : 'text-slate-600 hover:bg-white'}`}>{item.title}</button>)}
        </div>
...
function TableSection({ block }: { block: BuilderSiteBlock }) {
  ...
  const rows = block.config.tableRows || [];
  return (
    <SectionShell block={block}>
      <div className="overflow-x-auto rounded-3xl border border-slate-200 bg-white text-slate-900 shadow-sm">
        <table className="min-w-full border-collapse text-left text-sm">
          {columns.length > 0 && <thead><tr className="bg-slate-950 text-white">{columns.map((column, index) => <th key={`${column}-${index}`} className="px-5 py-4 font-black">{column}</th>)}</tr></thead>}
          <tbody>{rows.map((row, rowIndex) => <tr key={row.id || rowIndex} className={`border-t border-slate-100 ${block.config.tableStriped !== false && rowIndex % 2 ? 'bg-slate-50' : 'bg-white'}`}>{columns.map((_, cellIndex) => <td key={cellIndex} className="px-5 py-4 align-top text-slate-600">{row.cells[cellIndex] || ''}</td>)}</tr>)}</tbody>
        </table>
      </div>
    </SectionShell>
  );
  ...
  return (
    <SectionShell block={block}>
      <div className={`grid gap-5 ${columns}`}>
        {(block.config.teamItems || []).map((item, index) => {
...
function QuoteSection({ block }: { block: BuilderSiteBlock }) {
  return (
    <SectionShell block={block}>
      <figure className={`rounded-[2rem] border border-slate-200 bg-slate-50 p-7 text-slate-900 sm:p-10 ${block.config.alignment === 'center' ? 'mx-auto max-w-4xl text-center' : ''}`}>
        <QuoteIcon className={`h-10 w-10 text-indigo-300 ${block.config.alignment === 'center' ? 'mx-auto' : ''}`} />
        <blockquote className="mt-5 text-2xl font-black leading-tight tracking-tight sm:text-3xl">{block.config.quoteText || ''}</blockquote>
        {(block.config.quoteAuthor || block.config.quoteRole) && <figcaption className="mt-6 text-sm"><strong className="font-black text-slate-900">{block.config.quoteAuthor}</strong>{block.config.quoteRole && <span className="ml-2 text-slate-500">{block.config.quoteRole}</span>}</figcaption>}
      </figure>
    </SectionShell>
  );
}
