...
import { ArrowRight, CheckCircle2, Mail, Phone, Play } from 'lucide-react';
...
import type { GalleryDisplaySettings, GalleryImage } from '../../lib/galleryContent';
...
function actionNode(label: string, href: string, secondary = false): ReactNode {
  const className = secondary
    ? 'inline-flex items-center justify-center rounded-full border border-white/25 px-6 py-3 text-sm font-black text-white transition hover:bg-white/10'
    : 'inline-flex items-center justify-center rounded-full bg-white px-6 py-3 text-sm font-black text-slate-950 shadow-lg transition hover:-translate-y-0.5';
  if (href.startsWith('/') && !href.startsWith('//')) return <Link to={href} className={className}>{label}<ArrowRight className="ml-2 h-4 w-4" /></Link>;
  return <a href={href || '#'} className={className}>{label}<ArrowRight className="ml-2 h-4 w-4" /></a>;
}
...
function BuilderCases({ block }: { block: BuilderSiteBlock }) {
  ...
  return (
    <section id={block.config.anchor || undefined} className={`${spacingClass(block.config.spacing)} ${surfaceClass(block.config.style)}`}>
...
function BuilderContact({ block }: { block: BuilderSiteBlock }) {
  const { settings, l } = useSiteContent();
  const config = block.config;
  const phone = String(settings.phone || '').trim();
  const email = String(settings.email || '').trim();
  return (
    <section id={config.anchor || undefined} className={`${spacingClass(config.spacing)} ${surfaceClass(config.style || 'indigo')}`}>
      <div className={`mx-auto px-4 sm:px-6 lg:px-8 ${widthClass(config.width)}`}>
        <div className={`${config.alignment === 'left' ? '' : 'mx-auto text-center'} max-w-4xl`}>
          {config.badge && <div className="mb-3 text-xs font-black uppercase tracking-[0.2em] text-indigo-300">{config.badge}</div>}
          <h2 className="text-3xl font-black tracking-tight sm:text-5xl">{config.heading || l('Обговоримо ваш проєкт', 'Обсудим ваш проект', 'Let’s discuss your project')}</h2>
          {config.subheading && <p className="mt-5 text-base leading-8 text-slate-300 sm:text-lg">{config.subheading}</p>}
          {config.contactNote && <p className="mt-4 text-sm leading-7 text-slate-400">{config.contactNote}</p>}
          <div className={`mt-8 flex flex-wrap gap-3 ${config.alignment === 'left' ? '' : 'justify-center'}`}>
            {config.buttonText && actionNode(config.buttonText, config.buttonLink || '/contacts')}
            {config.secondaryButtonText && actionNode(config.secondaryButtonText, config.secondaryButtonLink || '/contacts', true)}
          </div>
          {(config.contactShowPhone !== false || config.contactShowEmail !== false) && (
            <div className={`mt-8 flex flex-wrap gap-3 ${config.alignment === 'left' ? '' : 'justify-center'}`}>
              {config.contactShowPhone !== false && phone && <a href={`tel:${phone.replace(/[^+\d]/g, '')}`} className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-bold text-white ring-1 ring-white/15 hover:bg-white/15"><Phone className="h-4 w-4" />{phone}</a>}
              {config.contactShowEmail !== false && email && <a href={`mailto:${email}`} className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-bold text-white ring-1 ring-white/15 hover:bg-white/15"><Mail className="h-4 w-4" />{email}</a>}
            </div>
          )}
        </div>
      </div>
...
export function PageBuilderRenderer({ block, preview = false }: PageBuilderRendererProps) {
  ...
  if (legacyKinds.has(kind)) return <DynamicBlockRenderer block={block} />;
  if (kind === 'testimonials') return <Testimonials />;
  if (kind === 'contact') return <BuilderContact block={block} />;
  if (kind === 'cases') return <BuilderCases block={block} />;
  if (kind === 'videos') return <BuilderVideos block={block} />;
  if (kind === 'divider') return <div id={config.anchor || undefined} className={`${config.spacerSize === 'large' ? 'py-10' : 'py-6'} bg-white`}>{config.dividerStyle === 'dots' ? <div className="text-center tracking-[0.8em] text-slate-300">•••</div> : <div className="mx-auto max-w-6xl border-t border-slate-200" />}</div>;
  if (kind === 'spacer') {
    ...
    return (
      <section id={config.anchor || undefined} className={`${spacingClass(config.spacing)} ${surfaceClass(config.style)}`}>
        <div className={`mx-auto px-4 sm:px-6 lg:px-8 ${widthClass(config.width)} ${config.alignment === 'center' ? 'text-center' : ''}`}>
          <SectionHeader block={block} />
          <RichTextRenderer value={config.richText || config.content || ''} className={`text-base leading-8 ${mutedClass(config.style)} [&_a]:font-semibold [&_a]:text-indigo-500`} />
        </div>
      </section>
    );
  }
  ...
  if (kind === 'image') {
    return (
      <section id={config.anchor || undefined} className={`${spacingClass(config.spacing)} ${surfaceClass(config.style)}`}>
        <div className={`mx-auto px-4 sm:px-6 lg:px-8 ${widthClass(config.width)}`}>
          <SectionHeader block={block} />
          {config.imageUrl && <figure className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 text-slate-900 shadow-sm"><ResponsiveImage src={config.imageUrl} alt={config.imageAlt || config.heading || ''} displayWidth={2000} sizes="100vw" className="max-h-[820px] w-full object-cover" />{config.imageCaption && <figcaption className="px-5 py-3 text-sm text-slate-500">{config.imageCaption}</figcaption>}</figure>}
        </div>
      </section>
    );
  }
  ...
  if (kind === 'gallery') {
    const galleryImages: GalleryImage[] = (config.galleryImages || []).filter(image => image.url).map(image => ({ ...image, id: image.id || image.url }));
    const gallerySettings: Partial<GalleryDisplaySettings> = {
      layout: config.galleryLayout || 'masonry',
      columns: config.galleryColumns || 3,
      gap: config.galleryGap || 'medium',
      aspect: config.galleryAspect || 'original',
      captionMode: config.galleryCaptionMode || 'always',
    };
    return (
      <section id={config.anchor || undefined} className={`${spacingClass(config.spacing)} ${surfaceClass(config.style)}`}>
        <div className={`mx-auto px-4 sm:px-6 lg:px-8 ${widthClass(config.width)}`}>
          <SectionHeader block={block} />
          <GalleryGrid images={galleryImages} galleryTitle={config.heading || block.title} settings={gallerySettings} />
        </div>
      </section>
    );
  }
